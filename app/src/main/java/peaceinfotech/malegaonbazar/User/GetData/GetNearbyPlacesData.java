...
//
//    public GetNearbyPlacesData (FragmentActivity delegate){
...
//    }
    ...
    private void showNearbyPlaces(List<HashMap<String,String>>nearbyPlaceList){
        ...
        for (int i=0;i<nearbyPlaceList.size();i++){
            ...
            if(click.equalsIgnoreCase("restaurant")) {
                LatLng latLng = new LatLng(lat, lng);
                markerOptions.position(latLng);
                markerOptions.title(placeName);
                markerOptions.snippet(vicinity);
                markerOptions.icon(BitmapDescriptorFactory.fromResource(R.drawable.rest_marker));
...
//                });
            ...
            else if (click.equalsIgnoreCase("hospital")){

                LatLng latLng = new LatLng(lat, lng);
                markerOptions.position(latLng);
                markerOptions.title(placeName);
                markerOptions.snippet(vicinity);
                markerOptions.icon(BitmapDescriptorFactory.fromResource(R.drawable.hosp_marker));
                mMap.addMarker(markerOptions);
                mMap.moveCamera(CameraUpdateFactory.newLatLng(latLng));
                mMap.animateCamera(CameraUpdateFactory.zoomTo(15));
//                mMap.setOnMarkerClickListener(new GoogleMap.OnMarkerClickListener() {
...
//                });
            ...
            else if(click.equalsIgnoreCase("school")){
